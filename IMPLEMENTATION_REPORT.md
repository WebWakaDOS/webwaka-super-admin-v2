...
**Branch:** `feature/phase-0-1-complete` → PR #4  
...
| Zod input validation | MEDIUM | Add to all POST/PUT endpoints (currently only login validated) |
| Integration tests | MEDIUM | Vitest + MSW tests for auth/tenants/billing flows |
| PWA icons | LOW | `/icons/icon-{192,512}.png` not yet present in `public/`; needed for installability |
| Workbox service worker | LOW | `sw.js` exists but lacks background sync for pendingMutations |
| Mobile sidebar | LOW | No drawer/hamburger on small screens yet |
...
**PR #4:** https://github.com/WebWakaDOS/webwaka-super-admin-v2/pull/4

Branch: `feature/phase-0-1-complete` → `master`
